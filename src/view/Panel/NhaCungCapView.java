...
public class NhaCungCapView extends JPanel implements ActionListener {
    ...
    private void initContent() {
        ...
        String[] cols = {"Mã NCC", "Tên Nhà Cung Cấp", "Số Điện Thoại", "Email", "Địa Chỉ"};
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == btnAdd) {
            JOptionPane.showMessageDialog(this, "Chức năng Thêm NCC đang được xây dựng!");
        } else if (e.getSource() == btnEdit) {
            if (table.getSelectedRow() == -1) { JOptionPane.showMessageDialog(this, "Vui lòng chọn NCC!"); return; }
            JOptionPane.showMessageDialog(this, "Chức năng Sửa đang được xây dựng!");
        } else if (e.getSource() == btnDelete) {
            if (table.getSelectedRow() == -1) { JOptionPane.showMessageDialog(this, "Vui lòng chọn NCC!"); return; }
            int cf = JOptionPane.showConfirmDialog(this, "Xác nhận xóa?", "Xóa", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
            if (cf == JOptionPane.YES_OPTION) JOptionPane.showMessageDialog(this, "Đã xóa!");
        } else if (e.getSource() == btnDetail) {
            if (table.getSelectedRow() == -1) { JOptionPane.showMessageDialog(this, "Vui lòng chọn NCC!"); return; }
            JOptionPane.showMessageDialog(this, "Chi tiết đang được xây dựng!");
        }
    }
}