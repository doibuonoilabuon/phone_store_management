...
public class MenuTaskbar extends JPanel {
    ...
    TrangChu trangChu; // Cập nhật tên lớp nếu cần
    SanPhamView sanPham;
    ...
    Main main;
    TaiKhoan user;
    public itemTaskbar[] listitem; // Cần đảm bảo file itemTaskbar.java không bị lỗi
    ...
    private void initComponent() {
        ...
        listitem[0].addMouseListener(new MouseAdapter() {
            @Override
            public void mousePressed(MouseEvent evt) {
                trangChu = new TrangChu(); // Gọi đúng tên class
    ...
    // Hàm in4 đã được CẮT BỎ CÁC CÂU LỆNH GÂY LỖI
    public void in4(JPanel info) {
        ...
        // Gán tĩnh icon (Cần đảm bảo file svg tồn tại, nếu không có thể để trống hoặc dùng icon mặc định của Java)
        try {
            lblIcon.setIcon(new FlatSVGIcon("./icon/man_50px.svg"));
        } catch(Exception e){
            // Bỏ qua lỗi hiển thị ảnh nếu đường dẫn sai
        }
        pnlIcon.add(lblIcon);
        ...
        // Gán tĩnh tên người dùng
        lblUsername = new JLabel("Người dùng hệ thống");
